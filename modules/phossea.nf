...
*/
process pssm_background_scores {

    cpus "${params.n_cores}"

    memory "16G"

    publishDir "${out_dir}", pattern: "PhosSEA/*.tsv.gz", mode: 'copy'
    publishDir "${out_dir}", pattern: "PhosSEA/*.h5", mode: 'copy'
    ...
    output:
        path "PhosSEA/*.tsv.gz", emit: tsvgz
        path "PhosSEA/*.h5", emit: h5
    ...
    pssm_background_scores.py \
        ...
        input/pssm_dict.h5 \
        ${params.n_cores} \
        PhosSEA/pssm_background_scores.h5 \
        | gzip > PhosSEA/pssm_background_scores.tsv.gz
    """
