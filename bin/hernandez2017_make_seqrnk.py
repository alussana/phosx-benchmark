...
def main():
    ...
    ac_res_pos_score_df = df.drop('ResiduePosition', axis=1)
    
    ac_res_pos_score_df = ac_res_pos_score_df.dropna()
    
    ac_res_pos_score_df['Position'] = ac_res_pos_score_df['Position'].astype(int)
    
    seqrnk = ac_res_pos_score_df.apply(make_seqrnk_entry, axis=1)
